...
export async function updateUser(
  ...
  if (updates.username && !/^[a-zA-Z0-9_]+$/.test(updates.username)) {
    ...
    throw new Error("Username must only consist of letters, numbers and underscore.");
  }

  if (updates.username) {
    // // Strip spaces and lowercase for consistency (optional)
    // let baseUsername = updates.username.trim();

    // // Remove any existing domain to avoid duplication
    // if (baseUsername.includes("@")) {
    //   baseUsername = baseUsername.split("@")[0];
    // }

    // // Append the fixed domain
    // updates.username = `${baseUsername}@snapcore.subspace.site`;
...
export async function scanUsers() {
  ...
  return (result.Items || []).map(
    ({ id, email, name, encryptedPrivateKey, publicKey, ...rest }) => rest
  );
}
...
export async function searchUsersByQuery(query: string, limit: number = 20) {
  const allUsersCmd = new ScanCommand({
    TableName: tableName,
    FilterExpression:
      "attribute_exists(username) AND attribute_exists(displayName) AND #act = :activated",
    ExpressionAttributeNames: { "#act": "activated" },
    ExpressionAttributeValues: { ":activated": true },
  });

  const allUsersResult = await getDynamoClient().send(allUsersCmd);
  const users = (allUsersResult.Items || []).map(
    ({ id, email, name, encryptedPrivateKey, publicKey, ...rest }) => rest
  );
